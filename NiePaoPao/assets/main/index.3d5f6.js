    ...
    execute: function () {
      ...
      var Game_GamePage_Logic = exports('default', (_dec = addLogic(Game_GamePage), _dec(_class = /*#__PURE__*/function (_GUILogicBase) {
        ...
        _proto.initBubbleGrid = function initBubbleGrid() {
          var _this3 = this;
          // 清空容器
          var bg = this.UI.m_BubbleArea.getChildAt(0);
          this.UI.m_BubbleArea.removeChildren();
          if (bg) {
            this.UI.m_BubbleArea.addChild(bg);
          }
          this.bubbles = [];
    ...
    execute: function () {
      ...
      var ToyControlInterface = exports('ToyControlInterface', /*#__PURE__*/function () {
        ...
        ToyControlInterface.vibratePhone = function vibratePhone() {
          if (!this.isEnabled) return;
          var now = Timer.currentTime; // 秒
          var elapsed = now - this.lastPhoneVibrateTime; // 秒

          // 防抖：如果距上次震动不足0.5秒，忽略
          if (elapsed < this.PHONE_VIBRATE_DURATION) {
            return;
          }
      ...
      // 手机震动控制
      ToyControlInterface.PHONE_VIBRATE_DURATION = 0.2;
      // 固定震动时长（秒，对应500ms）
